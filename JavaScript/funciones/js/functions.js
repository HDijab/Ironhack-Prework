...
function dniLetter( dni ) {
	...
	if (!(typeof dni === "number" || length === 8)) {
		return "Invalid parameter";
	};
	
	var result = lookup[dni % 23];
...
function findLongestWord( array_words ){
	var result = 0;
	for (name in array_words) {
		if (array_words[name].length > result) {
			result = array_words[name].length;
		}
	}
	return result

	//Another Solution
	//var result = array_words.sort(function (a, b) {return b.length - a.length;})[0];
...
array_lengths.push( findLongestWord( ["Batman", "Joker", "Harley", "Catwoman"] ) );
array_lengths.push( findLongestWord( ["One", "Two", "Three", "Four"] ) );
array_lengths.push( findLongestWord( ["Hi", "My", "Name", "Is", "Shady"] ) );
...
//EXERCISE 5
//
// Restore the comments marks (/* */) of the previous exercises. Exercises 3 and 4 have to be commented
// Uncomment the exercise code deleting /* and */ chars
// Open the console and reload the index page to see the result of the code.
...
function charFreq( string ) {
	...
	for (letter in array) {
		if (freqCounter.hasOwnProperty(array[letter])) {
			freqCounter[array[letter]]++;
		} else {
			freqCounter[array[letter]] = 1;
		}
	}
...
function beautifyLetters( array ) {
	for (letter in array) {
		if (/[aeiou]/.test(array[letter])) {
			array[letter] = array[letter].toUpperCase();
		} else if (/[^AEIOU0-9]/.test(array[letter])){
			array[letter] = array[letter].toLowerCase();
		}
	}
	return array;
	...
	var clean = function(array) {
		for (item in array) {
			...
			if (array[item] > 9) {
				store += array[item];
				array.splice(item, 1);
				store = store.toString().split('');
				for (i in store) {
	    			sum += parseInt(store[i],10);
	    		}
	    		array.unshift(sum);
			}
		}
